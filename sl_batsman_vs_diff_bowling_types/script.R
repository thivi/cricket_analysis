...
batsmen <- fast_bowl %>%
  ...
  mutate(
    sr = runs / balls * 100,
    avg = ifelse(is.infinite(runs / outs), runs, runs / outs)
  )
...
plot <- ggplot(batsmen, aes(x = avg, y = sr, label = bat)) +
  ...
  geom_text_repel() +
  geom_hline(
    yintercept = mean(batsmen$sr),
    linetype = "solid",
    color = "red"
  ) + # Average strike rate line
  geom_vline(
    xintercept = mean(batsmen$avg, na.rm = TRUE),
    linetype = "solid",
    color = "blue"
  ) + # Average runs line
  labs(
    x = "Average Runs",
    y = "Strike Rate",
    title = "Average Runs vs. Strike Rate"
  ) + # Labels and title
  theme_minimal()
print(plot)
